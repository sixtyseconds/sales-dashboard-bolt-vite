...
app.get('/api/contacts/:id/tasks', async (req, res) => {
  try {
    ...
    // For now, we'll create tasks based on activities and deals
    // In a real system, you'd have a dedicated tasks table
    const tasksQuery = `
      SELECT 
        'activity' as source,
        a.id::text as id,
        a.type || ' - ' || COALESCE(a.title, a.description, 'Follow up') as title,
        COALESCE(a.notes, 'No description') as description,
        CASE 
          WHEN a.type = 'meeting' THEN 'high'
          WHEN a.type = 'call' THEN 'medium' 
          ELSE 'low'
        END as priority,
      ...
      SELECT 
        'deal' as source,
        d.id::text as id,
        'Follow up on ' || COALESCE(d.title, d.description, 'deal') as title,
