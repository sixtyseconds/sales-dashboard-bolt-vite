...
export function useTasks(filters?: TaskFilters) {
  ...
  const fetchTasks = useCallback(async () => {
    ...
    try {
      ...
      // Build the query with contact relations
      let query = supabase
        .from('tasks')
        .select(`
          *,
  ...
  // Get tasks grouped by contact
  const getTasksGroupedByContact = useCallback(() => {
    return tasks.reduce((acc, task) => {
      const contactKey = task.contact_id || 'unassigned';
      if (!acc[contactKey]) {
        acc[contactKey] = [];
      }
      acc[contactKey].push(task);
      return acc;
