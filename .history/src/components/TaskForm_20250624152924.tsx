...
import { format, addDays, addWeeks, addMonths, isToday, isTomorrow, startOfWeek, addMinutes, setHours, setMinutes } from 'date-fns';
...
const TaskForm: React.FC<TaskFormProps> = ({
  ...
  const handleDateSelect = (date: Date | undefined) => {
    ...
    if (date && selectedTime) {
      ...
      const dateWithTime = setMinutes(setHours(date, parseInt(hours)), parseInt(minutes));
  ...
  const handleTimeChange = (time: string) => {
    ...
    if (selectedDate) {
      ...
      const dateWithTime = setMinutes(setHours(selectedDate, parseInt(hours)), parseInt(minutes));
      const formattedDate = format(dateWithTime, "yyyy-MM-dd'T'HH:mm");
  ...
  const getSmartQuickDates = () => {
    ...
    return [
      ...
      {
        label: 'End of Day',
        value: format(setMinutes(setHours(now, 17), 0), "yyyy-MM-dd'T'HH:mm"),
        icon: '🌅'
      },
      {
        label: 'Tomorrow 9 AM',
      ...
      {
        label: 'Next Monday',
        value: format(setMinutes(setHours(nextWeek, 9), 0), "yyyy-MM-dd'T'HH:mm"),
        icon: '📆'
      }
    ];
  };
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    <div className="text-green-400 font-medium">
                      Due: {format(selectedDate, 'EEEE, MMMM do, yyyy')} at {format(setMinutes(setHours(new Date(), parseInt(selectedTime.split(':')[0])), parseInt(selectedTime.split(':')[1])), 'h:mm a')}
                    </div>
          ...
          <div className="space-y-3">
            <Label className="text-base font-medium text-white flex items-center gap-2">
              ...
              <SelectTrigger className="bg-gray-800/70 border-gray-600/50 h-12 text-base rounded-xl text-white">
               ...
               <SelectContent className="bg-gray-800 border-gray-600 text-white">
                 {/* Me Option */}
                 <SelectItem value={userData?.id || ''} className="text-white hover:bg-gray-700 focus:bg-gray-700">
                   <div className="flex items-center gap-3">
