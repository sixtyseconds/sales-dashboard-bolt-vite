...
const TaskForm: React.FC<TaskFormProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Handle contact assignments vs user assignments
      const isContactAssignment = formData.assigned_to.startsWith('contact-');
      const assignedContactId = isContactAssignment ? formData.assigned_to.replace('contact-', '') : undefined;
      
      // Get contact info if assigning to a contact
      const assignedContact = isContactAssignment 
        ? contacts.find(c => c.id === assignedContactId)
        : null;

      // For contact assignments, we'll assign to the current user but track the contact info
      const actualAssignedTo = isContactAssignment ? userData?.id || formData.assigned_to : formData.assigned_to;
      ...
      const taskData = {
        ...
        contact_id: assignedContactId || contactId || undefined,
        // Add notes for prospect assignments
        notes: isContactAssignment ? `Task assigned to prospect: ${assignedContact?.full_name || assignedContact?.email}` : undefined,
      };
      ...
      if (task) {
        ...
        if (isContactAssignment) {
          toast.success(`Task created and assigned to prospect: ${assignedContact?.full_name || 'contact'}`);
        } else {
          ...
          <div className="space-y-3">
            ...
            <Select value={formData.assigned_to} onValueChange={(value) => handleInputChange('assigned_to', value)}>
              ...
              <SelectContent>
                ...
                {contacts.length > 0 && contacts.slice(0, 8).map((contact) => (
                ...
                <SelectItem value="steve">
                  <div className="flex items-center gap-3">
                    ...
                    <span>Steve</span>
                    <Badge variant="outline" className="ml-2 text-xs bg-purple-500/20 text-purple-400 border-purple-500/30">
                      Senior
                    </Badge>
                  </div>
                </SelectItem>
