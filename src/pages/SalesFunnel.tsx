...
export default function SalesFunnel() {
  ...
  const funnelMetrics = useMemo(() => {
    ...
    const outboundCount = monthActivities.filter(a => a.type === 'outbound').length;
    const meetingsCount = monthActivities.filter(a => a.type === 'meeting').length;
    const proposalsCount = monthActivities.filter(a => a.type === 'proposal').length;
    const closedCount = monthActivities.filter(a => a.type === 'sale').length;

    // Calculate conversion rates
    const outboundToMeetingRate = meetingsCount > 0 ? Math.round((meetingsCount / outboundCount) * 100) : 0;
    const meetingToProposalRate = proposalsCount > 0 ? Math.round((proposalsCount / meetingsCount) * 100) : 0;
    const proposalToClosedRate = closedCount > 0 ? Math.round((closedCount / proposalsCount) * 100) : 0;
